...
{
	lastSwitchTime = QDateTime::currentSecsSinceEpoch();
}
...
void ActiveWindowTracker::updateListApps(timetrackerapps* window) {
	...
	{
		AppManager::AppHistoryEntry entry;
		entry.appName = lastWindowTitle;
		entry.timeInSeconds = currentWindowTime;
		appManager_.appHistory.push_back(entry);
	}
